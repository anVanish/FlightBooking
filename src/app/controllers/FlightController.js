...
class FlightController{
    ...
    check(req, res, next){
        ...
        if (from === 'Từ')
            ...
            message = 'Vui lòng chọn số lượng hành khách'
        else if (oneway !== 'on' && returnDate === '')
            message = 'Vui lòng chọn ngày về'
        else if (from === to)
            message = 'Điểm đến và điểm đi không được trùng' 
    ...
    book(req, res, next){
        ...
        GeneralRepo.ticketBooking(req.cookies.user.user_id, flight.flight_id, total, members, formattedDate, code)
        res.redirect('../user')
    }
}
